...
【用語定義 — 厳守】
- α（Alpha）= Bet ÷ (Pot + Bet) = ブラフの損益分岐フォールド率 ＝ コールに必要な最低エクイティ
- MDF（Minimum Defense Frequency）= 1 − α = 相手のブラフを防ぐための最低コール（ディフェンス）頻度
  ※ MDF は「フォールドしてよい頻度」ではなく「コールしなければならない最低頻度」
- [GTO数学]ブロックはPythonで事前計算済み。AIは数値を再計算せず、そのまま引用して解釈すること。
- 数値（bb・%・EV推定など）は[GTO数学]ブロックに記載のもののみ引用可。AIが自前で計算・推定した数値は一切使用しないこと。
- OOPはMDF目安より多めのフォールドが均衡に近い場合がある（ただしMDFの定義は変わらない）。
- MDF はリバースポットのみ言及する。フロップ/ターンではMDFに触れず、エクイティ・ドロー・レンジ構成で論じること。

各ハンドを以下の構造で必ず日本語で出力してください。

- spot_range: このポジション(IP/OOP)・分類・ボードでHeroが均衡上持ちうるレンジの概要（1〜2文）
- balance_note: [GTO数学]の数値をポジション・ストリート・相手テンデンシーで補正した均衡コメント（1〜2文、数値はそのまま引用可）
...
# カテゴリ基礎スコア（そのカテゴリが定義上どれだけ難しいか）
_CATEGORY_DIFFICULTY = {
    "bluff_catch":         0.9,   # 定義上コール/フォールドが拮抗
    "nice_fold":           0.85,  # 定義上フォールドが難しい判断
    "hero_aggression_won": 0.7,   # ブラフ実行 = 混合戦略
    "value_success":       0.5,   # 正解だが難易度は様々
    "bluff_failed":        0.3,
    "bad_fold":            0.3,
    "call_lost":           0.3,
    "fold_unknown":        0.25,
...
_STREET_WEIGHT = {"river": 1.0, "turn": 0.8, "flop": 0.6, "preflop": 0.4}

# V1: この2カテゴリのみナイスプレイ候補（定義上インディファレンス＝信頼できる難易度）
_NICE_PLAY_CATS_V1 = {"bluff_catch", "nice_fold"}

# ナイスプレイとして表示する最低スコア（0.5 = ターン以上 × bluff_catch が必要）
NICE_PLAY_THRESHOLD = 0.5


def _compute_difficulty(hand: dict) -> float:
    """カテゴリ × ストリート × α微補正で難易度スコア 0.0〜1.0 を返す"""
    clf      = hand.get("bluered_classification") or {}
    category = clf.get("category", "")
    base     = _CATEGORY_DIFFICULTY.get(category, 0.2)

    streets  = hand.get("streets") or {}
    street_w = 0.4
    for st in ("river", "turn", "flop", "preflop"):
        if streets.get(st):
            street_w = _STREET_WEIGHT.get(st, 0.5)
            break

    # gto_math のα値で微補正（±0.05以内、カテゴリ判断を覆さない）
    alpha_adj = 0.0
    gto_math  = hand.get("gto_math", "")
    if "α=" in gto_math:
        m = re.search(r'α=(\d+)%', gto_math)
        if m:
            alpha        = int(m.group(1)) / 100
            indifference = 1.0 - abs(2 * alpha - 1)
            alpha_adj    = (indifference - 0.5) * 0.1

    return min(round(base * street_w + alpha_adj, 3), 1.0)


def _compute_nice_play_score(hand: dict) -> float:
    """V1: bluff_catch / nice_fold かつ閾値以上の場合のみ難易度スコアを返す。対象外は 0.0"""
    clf      = hand.get("bluered_classification") or {}
    category = clf.get("category", "")
    if category not in _NICE_PLAY_CATS_V1:
        return 0.0
    return hand.get("difficulty_score", 0.0)
        ...
        # ── フォールド系・その他（相手のベットに対するMDF基準）──────────
            ...
            for a in reversed(actions):
                ...
                if a_act in ("bet", "raise") and a_amt:
                    ...
                    alpha    = last_bet / (pot_bb + last_bet)
                    mdf      = 1 - alpha
                    # MDFはリバーのみ表示（フロップ/ターンはエクイティが残るため目安にならない）
                    if street_name == "river":
                        mdf_msg = f"MDF基準={mdf:.0%}（Heroがフォールド）"
                        default_msg = f"α={alpha:.0%} / MDF={mdf:.0%}"
                    else:
                        mdf_msg = "Heroがフォールド"
                        default_msg = f"α={alpha:.0%}"
                    spot_map = {
                        "fold_unknown": ("フォールドスポット",      mdf_msg),
                        "nice_fold":    ("ナイスフォールドスポット", mdf_msg),
                        "bad_fold":     ("バッドフォールドスポット", mdf_msg),
                    }
                    spot_type, spot_msg = spot_map.get(category, ("ベットスポット", default_msg))
                    return (
                        f"[GTO数学] {spot_type} | {street_label} | "
                        f"ベット={last_bet:.1f}bb / ポット={pot_bb:.1f}bb | "
                        f"α={alpha:.0%} | {spot_msg}"
...
def build_hand_block(idx: int, hand: dict) -> str:
    ...
    pos_str   = f"{pos}（{ip_oop}）" if ip_oop else pos
    cat_line  = f"\n分類: {category_label}" if category_label else ""
    math_line = f"\n{math}" if math else ""
    return f"""=== ハンド {idx} ===
ヒーロー: {pos_str} / 手札: {cards}
ボード: {board}{cat_line}{math_line}
アクション履歴:
{action}{ev_info}"""
...
- MDF（Minimum Defense Frequency）= 1 − α = 相手のブラフを防ぐための最低コール（ディフェンス）頻度
  ※ MDF は「フォールドしてよい頻度」ではなく「コールしなければならない最低頻度」
- [GTO数学]ブロックはPythonで事前計算済み。数値を再計算せず、そのまま引用して解釈すること。
- 数値（bb・%・EV推定など）は[GTO数学]ブロックに記載のもののみ引用可。AIが自前で計算・推定した数値は一切使用しないこと。
- MDF はリバースポットのみ言及する。フロップ/ターンではMDFに触れず、エクイティ・ドロー・レンジ構成で論じること。
...
- 箇条書きと段落を組み合わせて読みやすく
- [GTO数学]ブロックがある場合はその数値を解釈に使う（再計算不要・数値はそのまま引用）
