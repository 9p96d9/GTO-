...
def classify_result_page(
    ...
    _SUIT_COLORS_EARLY = {chr(0x2660): '#000000', chr(0x2665): '#e53935', chr(0x2666): '#1e88e5', chr(0x2663): '#43a047'}
    def _card_html(s):
        if not s: return ""
        import re as _re2
        def _r(m):
            c = _SUIT_COLORS_EARLY.get(m.group(2), '#000')
            return f'{_esc(m.group(1))}<span style="color:{c}">{m.group(2)}</span>'
        return _re2.sub(r'([23456789TJQKA]{1,2})([' + chr(0x2660) + chr(0x2665) + chr(0x2666) + chr(0x2663) + r'])', _r, str(s))
  ...
  <div class="summary-ev" style="padding:8px 20px;background:#fff;border-bottom:1px solid #e8e8e8;font-size:12px;color:#333">
    ...
    def _build_hand_card(h):
        ...
        _cat_label = clf.get("category_label", "")
        data_attrs = (
        ...
        return (
            f'<div class="{card_cls}" id="hcard-{hnum}" data-hnum="{hnum}" data-line="{line}"{data_attrs}>'
...
<div class="section">
  ...
  <div class="accordion-body collapsed" id="all-hands-body">
    ...
    return _env.get_template('classify_result.html').render(
        ...
        pf_count=pf_count,
        ev_html=ev_html,
        nice_plays_html=nice_plays_html,
        cat_rows=cat_rows,
