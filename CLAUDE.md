...
**「ナイスプレイをピックアップして先頭に表示する」**

ポーカーツールは「下手なプレイの振り返り」として使われることが多いが、それは心理的に辛く継続動機を下げる。
このツールは逆に「うまくできた判断」を先に見せることで、ポジティブな体験として学習を促す。
機能の優先順位・UI設計・AI解析の論調はこの哲学から逆算する。

---

## カスタムSKILL（スラッシュコマンド）

`.claude/commands/` に以下のSKILLが用意されている。会話内で `/skill-name` で呼び出せる。

| コマンド | 用途 |
|---|---|
| `/poker-brain` | AI解析品質チェック・プロンプト改善・GTO概念整理 |
| `/infra` | EC2・Docker・デプロイ・DB操作のクイックリファレンス |
