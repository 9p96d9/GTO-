...
analysis_hands (id SERIAL PK, analysis_id INT FK→analyses.id ON DELETE CASCADE NOT NULL,
                hand_number INT NOT NULL,
                line VARCHAR(10) NOT NULL,         -- 'blue' | 'red' | 'preflop'
                category_label VARCHAR(100) NOT NULL,
                position VARCHAR(10),              -- 'BTN' | 'SB' | 'BB' | 'CO' | 'HJ' | 'UTG'
                captured_at TIMESTAMPTZ)           -- ハンド取得日時（週集計に使用）
...
CREATE INDEX ON analysis_hands(analysis_id);
CREATE INDEX ON analysis_hands(line, position, captured_at);  -- 3D集計用
...
| 19-B  | 3D可視化リアルタイム化（スナップショット非依存） | ⬜ |

### PostgreSQL移行で実現できる機能（価値順）

Firestoreでは全件取得→Python計算が必要だったものが、SQLで1クエリに置き換わる。

| 優先度 | 機能 | Firestoreの課題 | SQLで解決 |
|---|---|---|---|
| ★★★ | 全ユーザー横断カテゴリ集計（アドミンKPI） | 全件読み込みで高コスト | GROUP BY user + category |
| ★★★ | 3D可視化のリアルタイム再集計 | スナップショット依存 | 週×ポジション×カテゴリを3軸GROUP BY |
| ★★☆ | 先週比で赤線率が悪化したユーザー検出 | 期間比較クエリが実質不可 | サブクエリで前週差分を算出 |
| ★★☆ | ポジション別損益ヒートマップ（全ユーザー） | コレクションまたぎ不可 | JOIN 1本で全ユーザー集計 |
| ★☆☆ | BBサイズ別バブルチャート（avg/stddev） | AVG・STDDEVは自前計算 | GROUP BY bb_size |
| ★☆☆ | ユーザー間ランキング | ソート軸が1フィールド限定 | ORDER BY + RANK() |

**実装順（★★★から着手）:**

1. **全ユーザーKPIダッシュボード** — `/admin` に集計カード追加
   - アクティブユーザー数・総ハンド数・カテゴリ分布をSQLで1クエリ取得
   
2. **3D可視化リアルタイム化** — スナップショット依存をやめて生データから再集計
   - `hands` テーブルから `week × position × category` の3軸GROUP BYで描画
   
3. **悪化ユーザー検出** — アドミンがフォローすべきユーザーを自動特定
   - 赤線率が前週比+10%以上のユーザーをリストアップ

### 19-A: KPIアナリティクスダッシュボード

**URL:** `/admin/analytics`（PostgreSQLモード限定）  
**ファイル変更:** `scripts/postgres_utils.py`, `routes/admin.py`, `templates/admin_analytics.html`（新規）

#### バックエンド：`get_admin_analytics()` → `postgres_utils.py`

```sql
-- ① 全ユーザーblue/red率ランキング（RANK()はFirestore不可）
SELECT
  u.email,
...
-- ② 先週比赤線率悪化ユーザー（CTE＋期間比較はFirestore不可）
WITH this_week AS (
  SELECT user_id,
...
{
  "user_stats": [
    {"email": "...", "total_hands": 300, "blue_rate": 62.3, "red_rate": 24.1, "red_rank": 1}
  ],
  "worsened_users": [
    {"email": "...", "this_week_pct": 35.0, "last_week_pct": 22.0, "diff_pct": 13.0}
  ],
  "fetched_at": "..."
}
```

#### フロントエンド：`admin_analytics.html`

- ユーザー一覧テーブル（blue_rate棒グラフインライン・red_rate棒グラフインライン・RANKバッジ）
- 悪化ユーザーテーブル（赤ハイライト・diff列でソート済み）
- `/admin` ページに「アナリティクス →」リンクを追加

---

### 19-B: 3D可視化リアルタイム化

**前提:** 19-10（`analysis_hands`テーブル追加）・19-11（`save_analysis`更新）が完了していること。

**URL:** 既存 `/3d_view/{job_id}` を拡張（`?mode=realtime`パラメータ追加）  
**ファイル変更:** `scripts/postgres_utils.py`, `routes/pages.py`, `templates/3d_view.html`

#### 19-10: Alembicマイグレーション

```bash
alembic revision --autogenerate -m "add analysis_hands"
# 生成されたversions/xxxx_add_analysis_hands.py を確認してapply
alembic upgrade head
```

#### 19-11: `save_analysis()`の`analysis_hands`INSERT

```python
# save_analysis() 内、analyses INSERT後に追加
for hand in hands:
    bc = hand.get("bluered_classification", {})
    line     = bc.get("line", "")
    label    = bc.get("category_label", "")
    position = hand.get("position", "")          # hero position
    cap_at   = hand.get("captured_at")           # ISO文字列 or None
    ...
    """), {"aid": analysis_id, "hnum": hnum, "line": line,
           "label": label, "pos": position, "cat": cap_at})
```

#### バックエンド：`get_3d_realtime(uid, job_id)` → `postgres_utils.py`

```sql
-- week × position × category の3軸GROUP BY（スナップショット不要）
SELECT
  DATE_TRUNC('week', ah.captured_at) AS week,
  ah.position,
  ah.category_label,
  ah.line,
  COUNT(*) AS hand_count
...
GROUP BY week, ah.position, ah.category_label, ah.line
ORDER BY week, ah.position;
```

#### フロントエンド

- `3d_view.html`：既存の`ALL_HANDS`データ注入に加え、PostgreSQLモード時は`/api/3d_view/{job_id}/realtime`エンドポイントからリアルタイム集計データを取得
- 取得データ形式は既存の3D可視化フォーマット（`week × position × category`）に変換してそのまま描画

---

### アドミンアナリティクスダッシュボード（旧仕様）
...
| 優先度 | 指標 | グラフ種別 |
|---|---|---|
| ★★★ | 全ユーザーカテゴリ別集計（blue/red/preflop比率） | ドーナツ + テーブル |
| ★★★ | 週×ポジション×カテゴリの3Dヒートマップ | 3Dバー（既存強化） |
| ★★☆ | 先週比赤線率悪化ユーザー一覧 | テーブル（アラート） |
| ★★☆ | ポジション別平均BB損益（全ユーザー） | ヒートマップ |
| ★☆☆ | BBサイズ別損益バブルチャート | バブルチャート |
| ★☆☆ | ユーザー別ハンド蓄積数ランキング | 棒グラフ |
